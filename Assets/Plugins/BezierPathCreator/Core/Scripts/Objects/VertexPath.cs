...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        #region ADS
        public float GetNearestPointDistanceOnTheCurve(Vector3 position, float precision)
        {
            if (precision <= 0)
                return 0;

            float nearestPointDistance = 0;
            float minDist = 0;

            for (float totalDist = 0; totalDist < length; totalDist+= precision)
            {
                Vector3 posOnCurve = GetPointAtDistance(totalDist);
                float distance = Vector3.Distance(position, posOnCurve);
                if (distance < minDist || minDist == 0)
                {
                    nearestPointDistance = totalDist;
                    minDist = distance;
                }
            }
        ...
        public Vector3 GetNearestPointDirectionOnTheCurve(Vector3 position, float precision)
        {
            if (precision <= 0)
                return Vector3.zero;
            ...
            return GetDirectionAtDistance(nearestPointDistance);
        }

        public Vector3 GetNearestPointDirectionOnTheCurve(Vector3 position, float precision, float minDistanceToBeAffected)
        {
            if (precision <= 0)
                return Vector3.zero;
            ...
            if (minDist > minDistanceToBeAffected)
                return Vector3.zero;
